	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getContentLength()<=0) {//Chay lan dau
			...
			RequestDispatcher rd= 
					request.getRequestDispatcher("ThemSanPham.jsp");
			rd.forward(request, response);
			return;
		}
		
		try {
			String  tensp = null, anh = null, gia = null, malsp = null;
...
//						System.out.println(tams);
			...
			int kq = spbo.add(tensp, anh, Long.parseLong(gia), Integer.parseInt(malsp));
			if(kq == 1)
				request.setAttribute("tb", "Thêm sản phẩm thành công!");
			else
				request.setAttribute("tb", "Thêm sản phẩm thất bại thất bại");
