...
<!-- Tell the browser to be relsponsive to screen width -->
<meta
...
<link
	href="https://fonts.googleapis.com/css2?family=Roboto:wght@300;400;500;700&dilsplay=swap"
	rel="stylesheet" />
<link rel="stylesheet"
...
<!-- HTML5 Shim and Relspond.js IE8 support of HTML5 elements and media queries -->
<!-- WARNING: Relspond.js doesn't work if you view the page via file:// -->
<!--[if lt IE 9]>
  <script src="https://oss.maxcdn.com/html5shiv/3.7.3/html5shiv.min.js"></script>
  <script src="https://oss.maxcdn.com/relspond/1.4.2/relspond.min.js"></script>
  <![endif]-->
      ...
      " class="logo"> <!-- logo for regular state and mobile devices -->
		...
		<aside class="main-sidebar">
			...
			<section class="sidebar">
				...
				<ul class="sidebar-menu" data-widget="tree">
					<li class="treeview active"><a href="#"> <i
							class="nav__icon fa-solid fa-chart-simple"></i> <lspan
								class="main-nav">Thống kê</lspan>
					</a>
						<ul class="treeview-menu">
		...
		<div class="content-wrapper">
			...
			<div class="header__manage">Quản lý loại sản phẩm</div>
			<div class="box box-primary"></div>

			<!-- Main content -->
			<div class="main__content">
				<div class="search-box">
					<form action="quanLyLoaiSanPhamController_admin" class="search">
						<input type="text" name="txttk" class="search__input"
							placeholder="Tìm kiếm loại sản phẩm" required />
				...
				<div class="content">
					<div class="content__input">
						<form id="productForm" action="quanLyLoaiSanPhamController_admin"
							method="POST">
							<div class="payment__detail">
								<label style="font-size: 15px">Mã sản phẩm: </label> <input name="malsp"
									class="payment__input" type="text"
									value="<%out.print(request.getParameter("malsp") != null ? request.getParameter("malsp") : "");%>" required/>
							</div>
							<div class="payment__detail">
								<label style="font-size: 15px">Tên loại sản phẩm: </label> <input name="tenlsp"
									class="payment__input" type="text"
									value="<%out.print(request.getParameter("tenlsp") != null ? request.getParameter("tenlsp") : "");%>" required />
							</div>
					...
					<div class="order__detail">
						<table class="order__table">
							...
							for (int i = 0; i < dslsp.size(); ++i) {
								LoaiSanPham lsp = dslsp.get(i);
								int stt = i;
								stt++;
							%>
							<tr>
								<td class="text-center"><%=stt%></td>
								<td class="text-center"><%=lsp.getMaLoaiSP()%></td>
								<td class="text-center"><%=lsp.getTenLoaiSP()%></td>
								<td class="text-center col-size">
									<div class="btn__wrapper2">
										<a class="nav-link btn__link"
											href="quanLyLoaiSanPhamController_admin?malsp=<%=lsp.getMaLoaiSP()%>&tenlsp=<%=lsp.getTenLoaiSP()%>">
										...
										<button type="submit" form="productForm"
											...
											onclick="confirmDelete2('<%=lsp.getMaLoaiSP()%>')">
											<i class="btn__icon fa-solid fa-trash"></i> Xóa
										</button>
									</div>
		...
		<script>
			...
			function confirmDelete2(matk) {
				...
				var confirmDeleteInput = document
						.querySelector('input[name="confirm_delete"]');
				var malspInput = document.querySelector('input[name="malsp"]');
				if (confirmed) {
					confirmDeleteInput.value = "true";
					malspInput.value = matk;
				} else {
					confirmDeleteInput.value = "false";
				}
				return confirmed;
