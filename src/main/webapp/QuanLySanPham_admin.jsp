	...
	<div class="wrapper">
      ...
      " class="logo"> <!-- logo for regular state and mobile devices -->
		...
		<div class="content-wrapper">
			...
			<div class="header__manage">Quản lý sản phẩm</div>
			<div class="box box-primary"></div>

			<!-- Main content -->
			<div class="main__content">
				<div class="search-box">
					<form action="quanLySanPhamController_admin" class="search">
				...
				<div class="content">
					<div class="content__input">
						<form id="productForm" action="quanLySanPhamController_admin"
							method="POST">
							...
							<div class="payment__detail">
								<label>Tên sản phẩm: </label> <input name="tensp"
									class="payment__input" type="text"
									value="<%out.print(request.getParameter("tensp") != null ? request.getParameter("tensp") : "");%>"
									autocomplete="off" required />
							</div>
							<div class="payment__detail">
								<label>Ảnh sản phẩm: </label> <input name="anhsp"
									class="payment__input" type="text"
									value="<%out.print(request.getParameter("anhsp") != null ? request.getParameter("anhsp") : "");%>"
									autocomplete="off" required />
							</div>
							<div class="payment__detail">
								<label>Giá bán: </label> <input name="giaban"
									class="payment__input" type="text"
									value="<%out.print(request.getParameter("giaban") != null ? request.getParameter("giaban") : "");%>"
									required />
							</div>
							<div class="payment__detail">
								<label>Mã loại sản phẩm: </label> <input name="maloaisp"
									class="payment__input" type="text"
									value="<%out.print(request.getParameter("maloaisp") != null ? request.getParameter("maloaisp") : "");%>"
									required />
							</div>
					...
					<div class="btn__wrapper">
						...
						</a> <a href="UpLoadFileController">
							<button type="submit" class="btn btn--save"
								name="action"  style="margin-left: 20px">
						...
						<button type="submit" form="productForm" class="btn btn--print"
							name="action" value="delete" onclick="return confirmDelete();" style="margin-left: 20px">
								...
								<td class="text-center col-size">
									<div class="btn__wrapper2">
										<%-- <a class="nav-link btn__link"
											href="quanLySanPhamController_admin?masp=<%=sp.getMaSanPham()%>&tensp=<%=sp.getTenSanPham()%>&anhsp=<%=sp.getAnhSanPham()%>&giaban=<%=sp.getGiaBan()%>&maloaisp=<%=sp.getMaLoaiSP()%>">
											<button class="btn btn--edit btn-new">
												<i class="btn__icon fa-solid fa-pen-to-square"></i>
											</button>
										</a> --%> <a class="nav-link btn__link"
											href="quanLySanPhamController_admin?masp=<%=sp.getMaSanPham()%>&tensp=<%=sp.getTenSanPham()%>&anhsp=<%=sp.getAnhSanPham()%>&giaban=<%=sp.getGiaBan()%>&maloaisp=<%=sp.getMaLoaiSP()%>">
											<button type="submit" class="btn btn--print btn-new"
												name="action" value="delete">
												<i class="btn__icon fa-solid fa-trash"></i>
											</button>
										</a>
									</div>
		...
		<script>
			function confirmDelete() {
				var confirmed = confirm("Bạn có chắc chắn muốn xóa sản phẩm?");
