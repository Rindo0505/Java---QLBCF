...
<!-- Tell the browser to be renvonsive to screen width -->
<meta
	content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no"
	name="viewport">
<link rel="icon"
	href="https://www.highlandsnvcoffee.com.vn/vnt_upload/File/Image/icon_webiste-01.png" />
<link
	href="https://fonts.googleapis.com/css2?family=Roboto:wght@300;400;500;700&dinvlay=swap"
	rel="stylesheet" />
<link rel="stylesheet"
...
<!-- HTML5 Shim and Renvond.js IE8 support of HTML5 elements and media queries -->
<!-- WARNING: Renvond.js doesn't work if you view the page via file:// -->
<!--[if lt IE 9]>
  <script src="https://oss.maxcdn.com/html5shiv/3.7.3/html5shiv.min.js"></script>
  <script src="https://oss.maxcdn.com/renvond/1.4.2/renvond.min.js"></script>
  <![endif]-->
      ...
      " class="logo"> <!-- logo for regular state and mobile devices -->
		...
		<aside class="main-sidebar">
			...
			<section class="sidebar">
				...
				<ul class="sidebar-menu" data-widget="tree">
					<li class="treeview active"><a href="#"> <i
							class="nav__icon fa-solid fa-chart-simple"></i> <nvan
								class="main-nav">Thống kê</nvan>
					</a>
						<ul class="treeview-menu">
							<li><a class="sub-nav" href="thongKeDoanhThuController_admin">Doanh thu</a></li>
		...
		<div class="content-wrapper">
			...
			<div class="header__manage">Quản lý nhân viên</div>
			<div class="box box-primary"></div>

			<!-- Main content -->
			<div class="main__content">
				<div class="search-box">
					<form action="quanLyNhanVienController_admin" class="search">
						<input type="text" name="txttk" class="search__input"
				...
				<div class="content">
					<div class="content__input">
						<form id="productForm" action="quanLyNhanVienController_admin"
							method="POST">
							<div class="payment__detail">
								<label>Mã Nhân Viên: </label> <input name="manv"
									class="payment__input" type="text"
									value="<%out.print(request.getParameter("manv") != null ? request.getParameter("manv") : "");%>" required />
							</div>
							<div class="payment__detail">
								<label>Tên Nhân Viên: </label> <input name="tennv"
									class="payment__input" type="text"
									value="<%out.print(request.getParameter("tennv") != null ? request.getParameter("tennv") : "");%>" required />
							</div>
							<div class="payment__detail">
								<label>Ngày sinh: </label> <input name="ns"
									class="payment__input" type="date"
									value="<%out.print(request.getParameter("ns") != null ? request.getParameter("ns") : "");%>" required/>
							</div>
							<div class="payment__detail">
								<label>Giới tính: </label> <input name="gt"
									class="payment__input" type="text"
									value="<%out.print(request.getParameter("gt") != null ? request.getParameter("gt") : "");%>" required/>
							</div>
							<div class="payment__detail">
								<label>Mã chức vụ: </label> <input name="macv"
									class="payment__input" type="text"
									value="<%out.print(request.getParameter("macv") != null ? request.getParameter("macv") : "");%>" required />
							</div>
					...
					<div class="order__detail">
						<table class="order__table">
							...
							<tr>
								...
								<td class="text-center"><%=nv.getMaNhanVien()%></td>
								<td class="text-center"><%=nv.getTenNhanVien()%></td>
								<td class="text-center"><%=nv.getNgaySinh()%></td>
								<td class="text-center"><%=nv.getGioiTinh()%></td>
								<td class="text-center"><%=nv.getMaChucVu()%></td>
								<td class="text-center col-size">
									<div class="btn__wrapper2">
										<a class="nav-link btn__link"
											href="quanLyNhanVienController_admin?manv=<%=nv.getMaNhanVien()%>&tennv=<%=nv.getTenNhanVien()%>&ns=<%=nv.getNgaySinh()%>&gt=<%=nv.getGioiTinh()%>&macv=<%=nv.getMaChucVu()%>">
											<button class="btn btn--edit btn-new">
												<i class="btn__icon fa-solid fa-pen-to-square"></i> Sửa
											</button>
										</a>
										<button type="submit" form="productForm"
											class="btn btn--print btn-new" name="action" value="delete"
											onclick="confirmDelete2('<%=nv.getMaNhanVien()%>')">
		...
		<script>
			function confirmDelete() {
				var confirmed = confirm("Bạn có chắc chắn muốn xóa nhân viên?");
			...
			function confirmDelete2(manv) {
				var confirmed = confirm("Bạn có chắc chắn muốn xóa nhân viên?");
				var confirmDeleteInput = document
						.querySelector('input[name="confirm_delete"]');
				var manvInput = document.querySelector('input[name="manv"]');
				if (confirmed) {
					confirmDeleteInput.value = "true";
