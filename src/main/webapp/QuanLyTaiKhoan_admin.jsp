...
<!-- Tell the browser to be retkonsive to screen width -->
<meta
...
<link
	href="https://fonts.googleapis.com/css2?family=Roboto:wght@300;400;500;700&ditklay=swap"
	rel="stylesheet" />
<link rel="stylesheet"
...
<!-- HTML5 Shim and Retkond.js IE8 support of HTML5 elements and media queries -->
<!-- WARNING: Retkond.js doesn't work if you view the page via file:// -->
<!--[if lt IE 9]>
  <script src="https://oss.maxcdn.com/html5shiv/3.7.3/html5shiv.min.js"></script>
  <script src="https://oss.maxcdn.com/retkond/1.4.2/retkond.min.js"></script>
  <![endif]-->
      ...
      " class="logo"> <!-- logo for regular state and mobile devices -->
		...
		<aside class="main-sidebar">
			...
			<section class="sidebar">
				...
				<ul class="sidebar-menu" data-widget="tree">
					...
					<li class="treeview active"><a href="#"> <i
						...
						<ul class="treeview-menu">
							...
							<li><a class="sub-nav sub-active" href="quanLyTaiKhoanController_admin?tdnadmin=<%=tendn%>">
									Tài khoản </a></li>
				...
				<ul class="sidebar-menu" data-widget="tree">
					<li class="treeview active"><a href="#"> <i
							class="nav__icon fa-solid fa-chart-simple"></i> <tkan
								class="main-nav">Thống kê</tkan>
					</a>
						<ul class="treeview-menu">
		...
		<div class="content-wrapper">
			...
			<div class="header__manage">Quản lý tài khoản</div>
			<div class="box box-primary"></div>

			<!-- Main content -->
			<div class="main__content">
				<div class="search-box">
					<form action="quanLyTaiKhoanController_admin" class="search">
						<input type="text" name="txttk" class="search__input"
							placeholder="Tìm kiếm tài khoản" required/>
				...
				<div class="content">
					<div class="content__input">
						<form id="productForm" action="quanLyTaiKhoanController_admin"
							method="POST">
							<div class="payment__detail">
								<label>Mã tài khoản: </label> <input name="matk"
									class="payment__input" type="text"
									value="<%out.print(request.getParameter("matk") != null ? request.getParameter("matk") : "");%>"
									autocomplete="off" required />
							</div>
							<div class="payment__detail">
								<label>Tên đăng nhập: </label> <input name="tendn"
									class="payment__input" type="text"
									value="<%out.print(request.getParameter("tendn") != null ? request.getParameter("tendn") : "");%>"
									autocomplete="off" required />
							</div>
							<div class="payment__detail">
								<label>Mật khẩu: </label> <input name="mk"
									class="payment__input" type="text"
									value="<%out.print(request.getParameter("mk") != null ? request.getParameter("mk") : "");%>"
									autocomplete="off" required />
							</div>
							<div class="payment__detail">
					...
					<%
					ArrayList<TaiKhoan> dstk = (ArrayList<TaiKhoan>) request.getAttribute("dstk");
					%>
					<div class="order__detail">
						<table class="order__table">
							...
							<tr>
								...
								<td class="text-center"><%=tk.getMaTaiKhoan()%></td>
								<td class="text-center"><%=tk.getTenDangNhap()%></td>
								<td class="text-center"><%=tk.getMatKhau()%></td>
								<td class="text-center"><%=tk.getMaNhanVien()%></td>
								<td class="text-center col-size">
									<div class="btn__wrapper2">
										<a class="nav-link btn__link"
											href="quanLyTaiKhoanController_admin?matk=<%=tk.getMaTaiKhoan()%>&tendn=<%=tk.getTenDangNhap()%>&mk=<%=tk.getMatKhau()%>&manv=<%=tk.getMaNhanVien()%>">
										...
										<button type="submit" form="productForm"
											...
											onclick="confirmDelete2('<%=tk.getMaTaiKhoan()%>')">
											<i class="btn__icon fa-solid fa-trash"></i> Xóa
										</button>
									</div>
		...
		<script>
			function confirmDelete() {
				var confirmed = confirm("Bạn có chắc chắn muốn xóa tài khoản?");
			...
			function confirmDelete2(matk) {
				var confirmed = confirm("Bạn có chắc chắn muốn xóa tài khoản?");
				var confirmDeleteInput = document
						.querySelector('input[name="confirm_delete"]');
				var matkInput = document.querySelector('input[name="matk"]');
				if (confirmed) {
					confirmDeleteInput.value = "true";
					matkInput.value = matk;
				} else {
					confirmDeleteInput.value = "false";
				}
				return confirmed;
